...
<a href={classOption.link} class={isFull ? 'disabled' : ''}>
  <div class="relative bg-white rounded-xl shadow-lgs p-8 flex flex-col justify-between text-center hover:scale-102 transition-transform duration-50 ease-in-out">
    ...
    {#if isFull}
    <button disabledc={isFull} class="mt-4 text-2xl bg-white text-[#EE6352] rounded-full hover:bg-brand-secondary uppercase font-bold py-2 rotate-[-10deg] m-auto">Class Full</button>
    {:else}
